...
function showPage(page) {
  const map = { home:'Homepage.html', index:'Homepage.html', modules:'Modules.html', quiz:'Quiz.html', tips:'Tips.html', about:'About.html' };
  const dest = map[page];
...
const MODULES = [
  {
    ...
    tag:"Polusi", tagClass:"tag-red", category:"polusi",
  ...
  },
  {
    ...
    tag:"Ekosistem", tagClass:"tag-green", category:"ekosistem",
  ...
  },
  {
    ...
    tag:"Iklim", tagClass:"tag-amber", category:"iklim",
  ...
  },
  {
    ...
    tag:"Aksi", tagClass:"", category:"aksi",
...
const QUIZ_DATA = {
  ...
  polusi:[
    ...
    {q:"Program apa yang bisa dilakukan masyarakat untuk langsung mengurangi polusi laut?",opts:["Menangkap ikan lebih banyak","Beach clean-up (bersih pantai)","Membuang sampah ke sungai","Mengurangi konsumsi ikan"],ans:1,expl:"Beach clean-up adalah kegiatan langsung yang efektif."},
  ...
  ekosistem:[
    {q:"Berapa luas terumbu karang Indonesia — terluas di dunia?",opts:["10.000 km²","30.000 km²","51.000 km²","100.000 km²"],ans:2,expl:"Indonesia memiliki terumbu karang seluas sekitar 51.000 km²."},
    ...
    {q:"Spesies apa yang membantu membersihkan alga di ekosistem karang?",opts:["Hiu","Ikan badut (nemo)","Bulu babi dan ikan kakaktua","Paus biru"],ans:2,expl:"Bulu babi dan ikan kakaktua (parrotfish) memakan alga yang mengancam karang."},
    {q:"Padang lamun (seagrass) penting bagi ekosistem laut karena...",opts:["Hanya untuk keindahan","Habitat dan sumber makanan dugong, penyu, dan banyak ikan","Menghasilkan garam","Menangkap ikan secara alami"],ans:1,expl:"Padang lamun adalah habitat vital bagi dugong, penyu hijau, dan ribuan spesies ikan."}
  ]
};
...
// Rating per user per modul — simpan di localStorage
function getRatingKey(moduleId) { return `og_rated_${moduleId}_${currentUser ? currentUser.username : 'guest'}`; }
function hasRated(moduleId) { return !!localStorage.getItem(getRatingKey(moduleId)); }
function markRated(moduleId, val) { localStorage.setItem(getRatingKey(moduleId), val); }
...
function updateNavUser() {
  ...
  if (!navAuth) return;
  if (currentUser) {
    navAuth.innerHTML = `<span style="font-size:0.85rem;color:var(--text-muted);margin-right:8px;">👤 ${currentUser.username}</span><button class="btn-outline" style="padding:7px 16px;font-size:0.82rem;" onclick="doLogout()">Keluar</button>`;
  } else {
    navAuth.innerHTML = `<button class="btn-primary" style="padding:8px 18px;font-size:0.85rem;" onclick="openAuth('login')">Masuk / Daftar</button>`;
  }
}
...
function renderModuleCard(m) {
  ...
  return `<div class="module-card reveal-card" onclick="openModule(${m.id})" style="animation-delay:${(m.id-1)*0.06}s">
...
// // RATING — 1 user 1 kali
// function renderRatingSection(m) {
//   updateStarInput(0, 'star-input');
//   const voted = hasRated(m.id);
//   const ratingContainer = document.getElementById('tab-rating');
...
//   const starInputRow = document.getElementById('star-input');
//   const submitBtn = ratingContainer.querySelector('.btn-primary');

  // if (voted) {
  //   const myVal = localStorage.getItem(getRatingKey(m.id));
  //   starInputRow.innerHTML = `<div class="rating-voted-msg">✅ Kamu sudah memberi rating ${myVal} bintang untuk modul ini. Terima kasih!</div>`;
  //   if (submitBtn) submitBtn.style.display = 'none';
  // } else {
  //   starInputRow.innerHTML = `
...
//   document.querySelectorAll('#' + id + ' .star-btn').forEach(b => {
//     if (b.dataset.val) b.classList.toggle('active', parseInt(b.dataset.val) <= val);
//   });
// }
// function submitRating() {
//   if (!currentRating) { showToast('⚠️ Pilih jumlah bintang terlebih dahulu!'); return; }
//   if (hasRated(currentModuleId)) { showToast('⚠️ Kamu sudah memberi rating untuk modul ini!'); return; }
...
//   markRated(currentModuleId, currentRating);
//   renderRatingSection(m);
...
function renderComments(m) {
  ...
  if (!form) return;
  if (!currentUser) {
    form.innerHTML = `<div class="comment-login-wall"><div style="font-size:2rem;margin-bottom:0.75rem;">🔒</div><p>Kamu harus <strong>masuk</strong> untuk bisa memberikan komentar.</p><button class="btn-primary" onclick="closeModal();openAuth('login')">Masuk / Daftar</button></div>`;
  } else {
    form.innerHTML = `
    <div class="comment-form">
      <div class="comment-name-input" style="background:var(--input-bg);border:1px solid var(--card-border);padding:10px 14px;border-radius:var(--radius-sm);font-size:0.88rem;color:var(--text-muted);">
        💬 Komentar sebagai <strong style="color:var(--ocean-main)">${currentUser.username}</strong>
      </div>
...
window.addEventListener('scroll', () => {
  ...
  if (nav) nav.style.background = window.scrollY>20 ? (isLight ? 'rgba(240,247,255,0.97)' : 'rgba(4,10,20,0.95)') : 'var(--nav-bg)';
});
